...
kNearest1 = cv2.ml.KNearest_create()
kNearest2 = cv2.ml.KNearest_create()
...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications1 = np.loadtxt("class_betu", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages1 = np.loadtxt("flattend_betu", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
    ...
    npaClassifications1 = npaClassifications1.reshape((npaClassifications1.size, 1))       # reshape numpy array to 1d, necessary to pass to call to train

    kNearest1.setDefaultK(1)                                                             # set default K to 1

    kNearest1.train(npaFlattenedImages1, cv2.ml.ROW_SAMPLE, npaClassifications1)           # train KNN object

    try:
        npaClassifications2 = np.loadtxt("class_szamok", np.float32)  # read in training classifications
    except:  # if file could not be opened
        print("error, unable to open classifications.txt, exiting program\n")  # show error message
        os.system("pause")
        return False  # and return False
    ...
    try:
        npaFlattenedImages2 = np.loadtxt("flattend_szamok", np.float32)  # read in training images
    except:  # if file could not be opened
        print("error, unable to open flattened_images.txt, exiting program\n")  # show error message
        os.system("pause")
        return False  # and return False
    ...
    npaClassifications2 = npaClassifications2.reshape(
        (npaClassifications2.size, 1))  # reshape numpy array to 1d, necessary to pass to call to train

    kNearest2.setDefaultK(1)  # set default K to 1

    kNearest2.train(npaFlattenedImages2, cv2.ml.ROW_SAMPLE, npaClassifications2)  # train KNN object
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    if len(listOfMatchingChars)==6:
        ...
        for currentChar in listOfMatchingChars:                                         # for each char in plate
            ...
            if counter < 3:
                retval, npaResults, neigh_resp, dists = kNearest1.findNearest(npaROIResized, k = 1)              # finally we can call findNearest !!!
            else:
                retval, npaResults, neigh_resp, dists = kNearest2.findNearest(npaROIResized, k = 1)              # finally we can call findNearest !!!
